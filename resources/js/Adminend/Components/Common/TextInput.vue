...
<template>
    <div>
        <label for="email" class="mb-1.5 block text-sm font-semibold text-gray-700 dark:text-gray-400" > {{ label }} </label>
        ...
        <div class="relative">
            <input v-model="model" :type="type" :placeholder="placeholder"
                ...
                :class="[ 'border rounded px-3 py-2 w-full transition duration-300 focus:outline-none', error ? 'border-red-500' : model && !error ? 'border-green-600' : 'border-gray-300' ]" />

            <span @click="togglePasswordVisibility" class="absolute z-30 text-gray-500 -translate-y-1/2 cursor-pointer ps-3 right-5 top-1/2 dark:text-gray-400 border-l border-gray-300" v-if="type==='password' || showPassword" >
